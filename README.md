...
| `user_providers` | 紀錄 Discord / Google 等第三方登入資訊 |
...
2. 使用者登入 (`POST /api/auth/login`) 或透過 Discord / Google 單點登入 (`/oauth/login/*`)，產生 `dl_session` cookie。
3. 第三方平台把使用者導向 `/oauth/authorize?client_id=...&redirect_uri=...&scope=...&state=...`
4. 使用者在授權頁面確認後，Dreamland 會把 `code` 帶回 `redirect_uri`。
...
| `/api/auth/login` | POST | 以 username 或 email 登入並寫入 `dl_session` cookie |
...
| `/api/oauth/authorize` | POST | 授權頁面送出時使用，回傳帶有 code 或 error 的 redirect URL |
